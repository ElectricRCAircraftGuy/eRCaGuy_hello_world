...
1. [`std::mutex` vs `std::lock_guard` vs `std::unique_lock` vs `std::scoped_lock` and `std::lock()`](#stdmutex-vs-stdlock_guard-vs-stdunique_lock-vs-stdscoped_lock-and-stdlock)
...
<a id="stdmutex-vs-stdlock_guard-vs-stdunique_lock-vs-stdscoped_lock-and-stdlock"></a>
# `std::mutex` vs `std::lock_guard` vs `std::unique_lock` vs `std::scoped_lock` and `std::lock()`
