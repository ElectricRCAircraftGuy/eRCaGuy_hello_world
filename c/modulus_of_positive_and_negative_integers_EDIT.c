...
```bash
meld c/modulus_of_positive_and_negative_integers.c c/modulus_of_positive_and_negative_integers_EDIT.c
```
...
/////////////////// add doxygen!
/// return the same answer as your programming calculator, or Google, which is
/// always a positive value, for instance, whenever num2 is positive and
/// if num1 is negative. unlike the `%` remainder operator in C!

/// Example:
///     So, `-5 % 180` in C or C++ is `-5`, but `mathematical_modulo(-5, 180)`
///     is the same as "-5 mod 180" in a calculator or on Google, and is `175`.
/// - This function correctly handles all cases where num1 and num2 can be
///   positive or negative. Ie: num1 positive and num2 negative, or num1
///   negative and num2 positive, or both negative, or both positive.
int mathematical_modulo(int num1, int num2)
{
    // This by itself correctly handles the cases where num1 and num2 are
    // **both positive**, and where num1 and num2 are **both negative**!
    int mod = num1 % num2;

    // Fix the special case where only `num1` is < 0 and the modulus is negative
    if (mod < 0 && num1 < 0 && num2 > 0)
    {
        mod += num2;
    }

    // Fix the special case where only `num2` < 0
    if (num2 < 0 && num1 > 0)
    {
        mod += num2;
    }
...
int main()
{
    ...
    // Another weird one. See: https://stackoverflow.com/q/4645897/4561887
    printf(" 25 %% -9 = %4i\n",  25 % -9);
    printf("-25 %% -9 = %4i\n", -25 % -9);
    printf("mathematical_modulo( 25, -9) = %4i\n",
            mathematical_modulo( 25, -9));
    printf("mathematical_modulo(-25, -9) = %4i\n",
            mathematical_modulo(-25, -9));
    printf("\n");
    ...
    // Unit tests
    // - Do a few unit tests to ensure we don't break the
    // `mathematical_modulo())` func later.
    ...
    // See: https://stackoverflow.com/q/4645897/4561887
    assert(mathematical_modulo(25, -9) == -2);
    assert(mathematical_modulo(-25, -9) == -7);
