...
1. Watch out for priority inheritance when using mutexes! This can unexpectedly cause a lower-priority task to run at a higher priority when it holds a mutex that a higher-priority task needs it to release. It will "inherit" the higher priority until it releases the mutex.
